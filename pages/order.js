        ...
        {Object.keys(products).map((key) => {
              return (
                <div key={key} className="border-2 justify-between mb-6 rounded-lg bg-white p-6 shadow-md sm:flex sm:justify-start">
                 <img src={products[key].img} alt="product-image" className="w-full h-72 rounded-lg sm:w-40 sm:h-40" />
            ...
            <div className="flex flex-col mt-5">
                 <p className="text-sm">Price - Rs.{products[key].price}/-</p>
                 <p className="text-sm">Qty - {products[key].qty}</p>
                
                {/*<p className="text-sm">Delivery Charge - Rs.{cart[k].del_ch}/-</p> */}
              </div>
            </div>
            <div className="mt-4 flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
              <div className="flex items-center border-gray-100">
                {/* <span onClick={()=>{removeFromCart(k,1,cart[k].price,cart[k].name,cart[k].size,cart[k].variant,cart[k].del_ch,cart[k].img)}} className="cursor-pointer rounded-l bg-gray-100 py-1 px-3.5 duration-100 hover:bg-blue-500 hover:text-blue-50"> - </span>
                <span className="mx-1">{cart[k].qty}</span>
                <span onClick={()=>{addToCart(k,1,cart[k].price,cart[k].name,cart[k].size,cart[k].variant,cart[k].del_ch,cart[k].img)}} className="cursor-pointer rounded-r bg-gray-100 py-1 px-3 duration-100 hover:bg-blue-500 hover:text-blue-50"> + </span>
