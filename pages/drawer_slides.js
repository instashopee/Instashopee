...
const drawer_slides = ({products}) => {
  return (
    <div>
      <section className="text-gray-600 body-font">
  <div className="container px-5 py-24 mx-auto ">
    <div className="flex flex-wrap -m-4 justify-center">
    {Object.keys(products).length==0 && <p>Sorry, All drawer_slides Out Of Stock, New drawer_slides Comming Soon !!</p>}
...
return <Link key={products[item].id} passHref={true} legacyBehavior href={`product/${products[item].slug}`}>
  <div className="lg:w-1/5 md:w-1/2 p-4 w-full cursor-pointer shadow-lg m-5 productCard transition-all">
    ...
    <div className="mt-4 text-center md:text-left">
      <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1 textPart">drawer_slides</h3>
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({category:' DRAWER SLIDES'})
  let drawer_slides = {}
  for(let item of products){
    if(item.title in drawer_slides){
        if(!drawer_slides[item.title].color.includes(item.color) && item.availableQty > 0){
          drawer_slides[item.title].color.push(item.color)
        }
        if(!drawer_slides[item.title].size.includes(item.size) && item.availableQty > 0){
          drawer_slides[item.title].size.push(item.size)
        }
    }

    else{
      drawer_slides[item.title] = JSON.parse(JSON.stringify(item))
      if(item.availableQty >0){
            drawer_slides[item.title].color = [item.color]
            drawer_slides[item.title].size = [item.size]
          }else{
            drawer_slides[item.title].color = []
            drawer_slides[item.title].size = []
          }
    }
  }

  return{
    props:{products:JSON.parse(JSON.stringify(drawer_slides))},
  }
  }
export default drawer_slides