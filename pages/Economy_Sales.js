...
export default function Economy_Sales({products3}){
  ...
  <div className="containerpy-3 mx-auto  ">
    ...
    {Object.keys(products3).length==0 && <p>Sorry, All Items Out Of Stock, New Products Comming Soon !!</p>}
...
return <Link key={products3[item].id} passHref={true} legacyBehavior href={`product/${products3[item].slug}`}>
  <div className="lg:w-[20rem] md:w-1/2 p-4 w-1/2 sm:h-[30rem] h-[27rem] sm:m-2 cursor-pointer shadow-lg  productCard transition-all">
     <div className='z-50 textPart'><span class="rounded-md bg-red-500 p-1 text-sm text-white ">{Math.floor(((products3[item].price-products3[item].mrp)/products3[item].mrp)*100)}% Off</span></div>
    ...
    <div className="mt-2 text-center md:text-left">
      ...
      ₹{products3[item].mrp}/-
        ...
        {products3[item].size.includes("14") && (<span className="border border-gray-300 px-1 mx-1">15</span>)}
        {products3[item].size.includes("15") && (<span className="border border-gray-300 px-1 mx-1">14</span>)}
        ...
        {products3[item].color.includes("rose gold") && (<button className="border-2 border-gray-300 rounded-full bg-none bg-[#B76E79] hover:bg-[#B76E79] w-6 h-6 focus:outline-none"></button>)}
